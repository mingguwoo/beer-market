...
public class BaseVehicleIdGenerator {
    ...
    public String createBaseVehicleId(String redisKey) {
        ...
        try {
            Object sequenceObject = redisTemplate.opsForValue().get(redisKey);
            if (ObjectUtils.isNotEmpty(sequenceObject)) {
                sequence = redisTemplate.opsForValue().increment(redisKey, CommonConstants.INT_ONE);
            }
        } catch (Exception e) {
        ...
        try {
            redisTemplate.opsForValue().set(redisKey, String.valueOf(sequence));
